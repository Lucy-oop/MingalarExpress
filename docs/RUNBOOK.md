...
| **Ports 54321/54322 belong to a different project.** | The running local stack is `Main_project` — an accounting schema (`invoices`, `payroll`, `tenants`). It is **not** Mingalar. Never point `.env.local` at it, and never run `supabase db reset` from a directory linked to it. |
| **There are SIX migrations, not five.** | `0001`–`0006`. `0006_settlement_admin.sql` carries the settlement lifecycle, `cod_positions`, `cod_by_shop` and `admin_overview` — the Super Admin panel is dead without it. |
...
Resets a throwaway container, applies all six migrations, seeds, and runs five
SQL suites. **Never** point it at anything else (§0).
